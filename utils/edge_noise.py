...
def add_edge_noise(
    ...
    if noise_type == "flip":
        ...
        # 元のエッジセットを作成（無向グラフとして）
        original_edges_set = set()
        for i in range(num_edges):
            src, dst = edge_index[0, i].item(), edge_index[1, i].item()
            # 無向グラフとして扱うため、小さい方を最初に
            if src < dst:
                original_edges_set.add((src, dst))
            else:
                original_edges_set.add((dst, src))
        
        # 削除するエッジ数を計算（元のエッジ数に対する割合）
        ...
        # 追加可能なエッジが削除数より少ない場合は、削除数を追加可能数に制限
        # これにより、削除と追加の数が一致する
        k_actual = min(k, len(possible_edges))
        
        # 削除数を実際の追加可能数に合わせる必要がある場合は、削除インデックスを再サンプリング
        if k_actual < k:
            # 削除するエッジ数を減らす（追加可能な数に合わせる）
            edge_indices_to_remove = np.sort(rs.choice(num_edges, size=k_actual, replace=False)).astype(np.int64)
            mask = np.ones(num_edges, dtype=bool)
            mask[edge_indices_to_remove] = False
            remaining_edges = edge_index[:, torch.tensor(mask, device=device)]
            # 既存のエッジセットを再計算
        ...
        if k_actual > 0:
            added_edge_indices = rs.choice(len(possible_edges), size=k_actual, replace=False)
            added_edges_list = [possible_edges[idx] for idx in added_edge_indices]
            
            # 双方向のエッジを追加
            added_edges_tensor = torch.zeros((2, k_actual * 2), dtype=torch.long, device=device)
        ...
        stats["added_edges"] = np.array(added_edge_indices) if k_actual > 0 else np.array([], dtype=np.int64)
