...
class FISF(torch.nn.Module):
    ...
    def propagate(self, x: Tensor, edge_index: Adj, mask: Tensor, mask_type: str, edge_weight: OptTensor = None) -> Tensor:
        ...
        k = max(1, min(int(feat_dim*self.gamma), feat_dim))
        low_var_index = torch.topk(torch.var(out, dim=0), k, largest=False).indices
    ...
    def compute_f_n2d(self, edge_index, feature_mask, mask_type, pre: OptTensor = None, feat_dim: OptTensor = None,
        ...
        if mask_type == 'structural':
            len_v_0tod_list = []
            f_n2d = torch.zeros(nv, dtype=torch.int, device=device)
            if pre == None:
            ...
            f_n2d = torch.zeros(feat_dim, nv, device=device)
            for i in range(feat_dim):
    ...
    def compute_edge_weight_c(self, edge_index, f_n2d, n_nodes):
        ...
        adj = torch.sparse_coo_tensor(edge_index, A_Dinv, size=[n_nodes, n_nodes], device=edge_index.device)
        return adj
